...
select distinct co.name from country co where co.year=2008 and (region='South America' or region ='Africa') order by co.population desc;
/* List all European countries (without duplicates) with a population more than one million (2,500,500) in year 2006. */
select distinct co.name from country co where co.year=2006 and (region='South America' or region ='Africa') and co.population > 1000000 order by co.population desc;
/* List the regions (without duplicates) of all countries starts with the letter "a" or "i" (lowercase) in their names. */
select distinct co.region from country co where substr(lower(co.name), 1, 1)='a' or substr(lower(co.name), 1, 1)='i' order by co.region, co.name asc;
/* List the regions (without duplicates) of all countries whose name ends with the letter "a" or "i". */
select distinct co.region from country co where right(co.name,1)='a' or right(co.name,1)='i' order by co.region, co.name asc;
...
--select distinct co.name, co.region, co.population, co.year as avgRegionPop from country co where co.name='' and co.year=''; /* Check if update worked. */
